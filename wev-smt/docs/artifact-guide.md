...
The artifact corresponds to commit `adb65a7` and the Day-13 packaging.
...
C1–C3 need no external data and run from the image alone (Functional badge). C4
needs a mounted `.litmus` corpus (Reusable badge); see README §4.
...
`reproduce.sh` (the image ENTRYPOINT) runs six steps and prints a checklist:

1. **Unit tests** — `mvn -o test`; asserts surefire totals are `tests=34,
   failures=0, errors=0`.
...
| 6 plots | <1 min | |
| **total (1–4 + plots)** | **< 30 min** | the AE time box |
...
* **Determinism scope.** Reproduced byte-for-byte: verdicts, match outcomes,
  hierarchy-soundness. **Not** reproduced exactly: timings, memory, the
  separation-atlas cell contents, and the absolute corpus file count (all
  machine/budget-dependent — by design, see §2).
...
```bash
cd wev-smt
mvn test                                   # 34 tests; copies the Z3 native into target/native
